-- Clean slate: drop everything from previous attempts, then recreate fresh.
drop table if exists patients cascade;
drop table if exists sessions cascade;
drop function if exists touch_session_updated_at() cascade;
...
-- Sanity check: run as anon right here, in the same script, so we know
-- immediately if it worked instead of going back to the app.
set role anon;
insert into sessions (id, status) values (gen_random_uuid(), 'active') returning id, status;
reset role;
